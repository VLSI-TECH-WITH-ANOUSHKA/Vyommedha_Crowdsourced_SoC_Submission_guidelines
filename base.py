...
_io = [
    ...
    # Button
    ("cpu_reset", 0, Pins("J15"), IOStandard("3.3-V LVTTL")),
...
class BaseSoC(SoCCore):
    def __init__(self, platform):
        ...
        self.submodules.crg = CRG(platform.request("clk50"), ~platform.request("cpu_reset"))
                # Led
        user_led = Cat(*[platform.request("user_led", i) for i in range(8)])
        self.submodules.leds = Led(user_led)
        self.add_csr("leds")

        # Switches
        user_switches = Cat(*[platform.request("sw", i) for i in range(4)])
